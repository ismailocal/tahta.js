...
export class ShapeRegistry {
  ...
  readonly #runtimes = new Map<string, unknown>();
  ...
  attachRuntime<Runtime>(type: string, runtime: Runtime): void {
    this.get(type);
    if (this.#runtimes.has(type)) {
      throw new CanvasValidationError(`Shape runtime '${type}' is already attached`, 'DUPLICATE_SHAPE_RUNTIME');
    }
    this.#runtimes.set(type, runtime);
  }

  hasRuntime(type: string): boolean {
    return this.#runtimes.has(type);
  }

  getRuntime<Runtime>(type: string): Runtime {
    this.get(type);
    const runtime = this.#runtimes.get(type);
    if (!runtime) {
      throw new CanvasValidationError(`Shape runtime '${type}' is not attached`, 'MISSING_SHAPE_RUNTIME');
    }
